...
function renderWeatherWidget() {
  ...
  fetch(
    "https://api.open-meteo.com/v1/forecast?latitude=49.0731&longitude=33.4198&current_weather=true&daily=temperature_2m_max,temperature_2m_min,weathercode,precipitation_sum&timezone=Europe%2FKiev&forecast_days=7"
  )
    .then((r) => r.json())
    .then((data) => {
      if (!data || !data.daily) {
      ...
      let html = `<div class=\"weather-today-main\" style=\"display:flex;align-items:center;gap:10px;margin-bottom:8px;justify-content:center;\">
        ...
        <div style=\"display:flex;flex-direction:column;align-items:flex-start;gap:2px;\">
          ...
          <span style=\"font-size:0.95em;color:#fff;\">Макс: <b>${Math.round(
            data.daily.temperature_2m_max[0]
          )}&deg;C</b>, Мін: <b>${Math.round(
        data.daily.temperature_2m_min[0]
      )}&deg;C</b></span>
          <span style=\"font-size:0.95em;color:#fff;\">Вітер: <b>${
            w.windspeed
          } м/с</b></span>
          <span style=\"font-size:0.95em;color:#fff;\">Опади: <b>${
            data.daily.precipitation_sum ? data.daily.precipitation_sum[0] : 0
          } мм</b></span>
        </div>
      </div>`;
      ...
      html +=
        '<div class="weather-city" style="margin-top:auto;text-align:center;font-size:1em;opacity:0.8;">Кременчук</div>';
      ...
      forecastDiv.style.cssText =
        "display: flex; gap: 8px; justify-content: center; flex-wrap: wrap; margin: 10px 0;";

      for (let i = 0; i < 7; i++) {
        const date = new Date(days[i]);
        const isToday = date.toDateString() === todayDate.toDateString();
        forecastDiv.innerHTML += `<div class=\"weather-day${
          isToday ? " today" : ""
        }\" data-idx=\"${i}\" style=\"
          background: ${
            isToday ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.1)"
          };
          border-radius: 8px;
          padding: 8px;
          text-align: center;
        ...
        \" onmouseover=\"this.style.background='rgba(255,255,255,0.3)'\" onmouseout=\"this.style.background='${
          ...
          <span class=\"icon\" style=\"font-size: 1.2em; display: block; margin-bottom: 4px;\">${getWeatherIcon(
            codes[i]
          )}</span>
          <span style=\"font-size:0.8em; display: block; margin-bottom: 2px;\">${
            weekDays[date.getDay()]
          }</span>
          <span style=\"font-size:0.8em; display: block; margin-bottom: 2px;\">${Math.round(
            maxT[i]
          )}&deg;/${Math.round(minT[i])}&deg;</span>
  ...
  content.innerHTML = `
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
      <div style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 8px;">
        ...
        <div style="font-size: 1.2em; font-weight: bold; color: #e74c3c;">${Math.round(
          data.daily.temperature_2m_max[0]
        )}°C</div>
      </div>
      <div style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 8px;">
        <div style="font-size: 0.9em; color: #666; margin-bottom: 5px;">Мінімум</div>
        <div style="font-size: 1.2em; font-weight: bold; color: #3498db;">${Math.round(
          data.daily.temperature_2m_min[0]
        )}°C</div>
      </div>
      <div style="text-align: center; padding: 15px; background: #f8f9fa; border-radius: 8px;">
